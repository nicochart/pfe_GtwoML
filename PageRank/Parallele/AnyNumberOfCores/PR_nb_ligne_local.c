/*Travail pour rendre indépendant le code du nombre de coeurs / dimensions en utilisant un nombre de ligne local et non global.
NON FONCTIONNEL : le MPI_Allgather ne fonctionne pas car certains messages sont tronqués (nb_ligne_local plus grand dans certains processus que dans d'autres)*/
/*PR Non pondéré parallèle*/
/*Nicolas HOCHART*/
...
int main(int argc, char **argv)
{
    ...
    n = atoll(argv[1]);
    size = n * n;
    long nb_ligne_local; //nombre de lignes dans le bloc local

    long nb_ligne_base = n/p;
    int reste_lignes_a_repartir = n%p;
    if (my_rank==0)
    {
        printf("Nombres de lignes par processus = n/p (p nombre de coeurs) = %i / %i = %f\n",n,p,(double) n / (double) p);
        if (reste_lignes_a_repartir != 0)
        {
            printf("Répartition des lignes = %i par processus + %i à répartir (donc 1 par processus pour les processus 0 à %i)\n",nb_ligne_base,reste_lignes_a_repartir,reste_lignes_a_repartir-1);
        }
    }

    if (my_rank < reste_lignes_a_repartir)
    {
        nb_ligne_local = nb_ligne_base + 1; //une ligne bonus parmis les lignes en trop
    }
    else
    {
        nb_ligne_local = nb_ligne_base;
    }
    MPI_Barrier(MPI_COMM_WORLD);
    if (debug) {printf("Nombre de lignes dans le processus %i = %i\n",my_rank,nb_ligne_local);}
    MPI_Barrier(MPI_COMM_WORLD);
    ...
    if (my_rank < reste_lignes_a_repartir) //on est dans les processus ayant potentiellement une ligne bonus
    {
        nb_lignes_deja_ecrites = my_rank*nb_ligne_local;
    }
    else
    {
        nb_lignes_deja_ecrites = my_rank*nb_ligne_local + /*éventuelles lignes bonus des processus d'avant*/ reste_lignes_a_repartir;
    }

    generate_coo_matrix_for_pagerank(&A_COO, nb_lignes_deja_ecrites, zeros_percentages[my_rank], nb_ligne_local, n);
    ...
    double to_add,sum_totale_old_q,sum_totale_new_q,sum_new_q,tmp_sum,sc,morceau_new_q[nb_ligne_local];
    ...
    while (error_vect > epsilon && !one_in_vector(new_q,n) && cpt_iterations<maxIter)
    {
        ...
        MPI_Allgather(morceau_new_q, nb_ligne_local, MPI_DOUBLE, new_q, nb_ligne_local,  MPI_DOUBLE, MPI_COMM_WORLD); //récupération des morceaux de new_q dans new_q, dans tout les processus
