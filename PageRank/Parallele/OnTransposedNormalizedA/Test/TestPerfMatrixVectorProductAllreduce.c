/*Programme permettant de tester la communication "Allreduce" du produit Matrice-Vecteur du PageRank appliqué à une matrice d'adjacence transposée.*/
/*Nicolas HOCHART*/
...
int main(int argc, char **argv)
{
    ...
    double *vector_local, *vector_global;
    long vector_size;
    ...
    vector_local = (double *)malloc(vector_size * sizeof(double));
    vector_global = (double *)malloc(vector_size * sizeof(double));

    for (cpt_iteration=0; cpt_iteration<50; cpt_iteration++)
    {
        for (i=0;i<vector_size;i++) {vector_local[i] = random_between_0_and_1(); vector_global[i] = 0;}

        MPI_Barrier(MPI_COMM_WORLD);
        start_time = my_gettimeofday(); //début mesure de temps

        /********* DEBUT COMMUNICATION ALLREDUCE DU PRODUIT MATRICE VECTEUR *********/
        MPI_Allreduce(vector_local, vector_global, vector_size, MPI_DOUBLE, MPI_SUM, RV_CALC_GROUP_COMM); //chaque processus d'une "colonne de processus" (dans la grille) contient le même vector de taille vector_size
        /********** FIN COMMUNICATION ALLREDUCE DU PRODUIT MATRICE VECTEUR **********/
