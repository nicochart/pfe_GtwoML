...
int main(int argc, char **argv)
{
    ...
    struct MatrixBlock myBlock; //contiendra toutes les informations du block local (processus)
    long long size;
    ...
    if (my_rank == 0 && debug)
    {
        printf("Vecteur \"nb_connections_rows_global\" = ");
        for (i=0; i<A_CSR.dim_c; i++)
        {
            printf("%li ",nb_connections_rows_global[i]);
        }
        printf("\n");
    }
    ...
    normalize_csr_binary_matrix_on_rows(&P_CSR, myBlock); //MatrixDebugInfo.nb_connections
    ...
    if (debug_cerveau)
    {
        for(i=0;i<n;i++) // Parcours des neurones
        {
            ...
            nbco = nb_connections_rows_global[my_rank*nb_ligne+i];
            if (my_rank == 0)
            {
    ...
    if (debug_cerveau)
    {
        free(nb_connections_rows_global); //reviens à free MatrixDebugInfo.nb_connections MatrixDebugInfo.types est free plus haut : free(neuron_types);
    }
    MPI_Finalize();
    return 0;
}
